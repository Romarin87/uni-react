...
class GeometricStructureTask(torch.nn.Module):
    """Atom mask + coordinate denoise + charge prediction."""
    ...
    @staticmethod
    def _normalize_subtasks(subtasks: Optional[Iterable[str]], default: Iterable[str]) -> Set[str]:
    ...
    def forward(
        ...
        if "atom_mask" in st:
            out.update(self.atom_mask(descriptors))
        if "coord_denoise" in st:
            out.update(self.coord_denoise(descriptors))
        if "charge" in st:
            out.update(self.charge(descriptors))
        return out
