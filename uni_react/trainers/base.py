"""BaseTrainer – shared bookkeeping for all training loops."""
import time
...
class BaseTrainer:
    ...
    Subclasses must implement :meth:`train_epoch` and :meth:`eval_epoch`.

    Args:
        model: The (unwrapped) model instance.
        optimizer: Configured optimizer.
        scheduler: LR scheduler (or ``None`` for constant LR).
        logger: A :class:`~uni_react.core.logger.LoggerProtocol` instance.
        out_dir: Directory for checkpoints and config dumps.
    ...
    def save_checkpoint(
        ...
        """Save model + optimizer state to ``out_dir/<tag>.pt``."""
    ...
    def load_checkpoint(
        ...
        target = self.raw_model
        target.load_state_dict(ckpt["model"], strict=strict)
        ...
        self.resume_step_in_epoch = int(ckpt.get("step_in_epoch", 0) or 0)
        epoch = int(ckpt.get("epoch", 0))
        return epoch if self.resume_step_in_epoch > 0 else epoch + 1
    ...
    def reduce_metrics(
        self, meters: Dict[str, float], steps: int
    ) -> Dict[str, float]:
        """Average metric accumulators across all ranks."""
        if not self.distributed:
            return {k: v / steps for k, v in meters.items()} if steps else meters

        keys = list(meters)
        values = torch.tensor(
            [meters[k] for k in keys] + [float(steps)],
            device=self.device,
            dtype=torch.float64,
        )
        dist.all_reduce(values, op=dist.ReduceOp.SUM)
        total_steps = int(values[-1].item())
        if total_steps == 0:
            return {k: 0.0 for k in keys}

        return {k: float(values[i].item() / total_steps) for i, k in enumerate(keys)}

    def _reduce_bag(self, bag: MetricBag) -> Dict[str, float]:
        """Reduce a :class:`MetricBag` across all ranks and return a plain dict."""
        sums = bag.get_sums()
        weight = bag.get_weight()
        return self.reduce_metrics(sums, int(weight))
