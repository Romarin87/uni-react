"""Dedicated QM9 trainer for GotenNet-style optimization."""
...
class _WarmupPlateauController:
    """Linear warmup per step, then ReduceLROnPlateau per epoch."""
...
class GotenNetQM9Trainer(FinetuneQM9Trainer):
    """QM9 trainer that mirrors the official GotenNet optimization style."""
    ...
    def load_checkpoint(
        ...
        next_epoch = super().load_checkpoint(
            path,
            strict=strict,
    ...
    def _new_metric_bag(self):
        from ..metrics import MetricBag

        return MetricBag(["loss", "mae"])
