"""Dataclass schema for geometric task runs."""
...
class GeometricConfig:
    """All hyper-parameters for a geometric structure task run.
    ...
    """Regression loss for coordinate/charge/electronic heads."""
    ...
    def __post_init__(self):
        ...
        valid_regression_losses = {"mse", "l1", "mae", "huber", "smooth_l1", "charbonnier", "log_cosh"}
        if self.regression_loss not in valid_regression_losses:
            raise ValueError(
                f"regression_loss must be one of {valid_regression_losses}, got {self.regression_loss!r}"
            )
        ...
        if self.charbonnier_eps <= 0:
            raise ValueError(f"charbonnier_eps must be > 0, got {self.charbonnier_eps}")
