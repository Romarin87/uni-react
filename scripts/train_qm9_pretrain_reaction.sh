...
# 第三阶段（reaction triplet）预训练权重初始化的 QM9 下游训练脚本。
...
if [[ -z "${PRETRAINED_CKPT}" ]]; then
  case "${ENCODER_TYPE}" in
    gotennet_l) PRETRAINED_CKPT="runs/gotennet_l_reaction/best.pt" ;;
    reacformer_se3) PRETRAINED_CKPT="runs/reacformer_se3_reaction/best.pt" ;;
    reacformer_so2) PRETRAINED_CKPT="runs/reacformer_so2_reaction/best.pt" ;;
    *) PRETRAINED_CKPT="runs/single_mol_reaction/best.pt" ;;
  esac
fi
...
if [[ -z "${OUT_PREFIX}" ]]; then
  case "${ENCODER_TYPE}" in
    single_mol) OUT_PREFIX="qm9_pretrain_reaction" ;;
    *) OUT_PREFIX="qm9_pretrain_${ENCODER_TYPE}_reaction" ;;
  esac
fi
export OUT_PREFIX
